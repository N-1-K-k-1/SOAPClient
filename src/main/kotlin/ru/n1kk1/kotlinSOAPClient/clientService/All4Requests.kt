...
class All4Requests : Runnable {
    ...
    fun updateCustomer(correlationId: Int) {
        ...
        response = correlationId.let { quoteClient.all4Requests(it.toLong()) }.serviceStatus.message + "\n" + response
        Logger.getAnonymousLogger().info("EXIT POST GET PUT DELETE@" + Date().time + " for correlationId = " + correlationId +  " from " + Thread.currentThread().name)
